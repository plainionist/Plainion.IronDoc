...
module internal Transformer = 
    ...
    let tryProcessPermission ctx (e:XElement) =
        if e <> null then
            ...
            ctx.Writer.Write ( e.Attribute(!!"cref").Value )
            ctx.Writer.WriteLine "*"
            ctx.Writer.WriteLine (Utils.NormalizeSpace e.Value )
    ...
    let processMemberDoc ctx (memb :XElement) (level :int) = 
        ...
        |> Seq.iter( fun s -> 
            ctx.Writer.WriteLine(Utils.NormalizeSpace s.Value )

            memb.Elements(!!"para")
            |> Seq.iter( fun p -> ctx.Writer.WriteLine(Utils.NormalizeSpace p.Value))
        )
        ...
        if memb.Elements(!!"remarks").Any() then
            ...
            |> Seq.iter( fun r -> 
                ctx.Writer.WriteLine ()
                ctx.Writer.Write "> "
                ctx.Writer.WriteLine (Utils.NormalizeSpace r.Value )
            )
        ...
        if memb.Elements(!!"param").Any() then
            ...
            |> Seq.iter( fun p -> 
                ...
                ctx.Writer.Write ":**  " 
                ctx.Writer.Write ( Utils.NormalizeSpace p.Value )
                ctx.Writer.WriteLine ()
            )
        ...
        if memb.Elements(!!"exception").Any() then
            ...
            |> Seq.iter( fun ex -> 
                ...
                let cref = Utils.SubstringAfter ( ex.Attribute(!!"cref").Value) ':' 
